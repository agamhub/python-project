CREATE PROC [FOND_ID].[USP_LOAD_ETL5_PRODUCTION_COMMISSION_DRIVER_DETAIL_YTD] @batch [NVARCHAR](100) AS
BEGIN 
	--DECLARE @batch [nvarchar](30);
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'FOND_ID.FOND_ETL5_PRODUCTION_COMMISSION_DRIVER_DETAIL';
...
BEGIN TRY
		...
		IF OBJECT_ID('tempdb..#etl5_production_commission_driver') IS NOT NULL
		BEGIN
			DROP TABLE #etl5_production_commission_driver
		END;

		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'SELECT INTO TEMPORARY TABLE etl5_production_commission_driver : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		...
		SELECT * 
		INTO #etl5_production_commission_driver
		FROM (
		SELECT 
				'IAI' ENTITY_ID,
				...
				SUM(CAST([PRODUCTION_BONUS]AS NUMERIC(18,3))) DRIVER_AMOUNT,
				'0' BATCH_MASTER_ID,
				'0' BATCH_RUN_ID,
				'0' JOB_MASTER_ID,
				'0' JOB_RUN_ID,
		...
		[FOND_ID].[FOND_ETL5_LIFEASIA_DRIVER_DETAIL_PRODUCTION_PERSISTENCY]
		...
		--AND PRODUCTION_BONUS IS NOT NULL
		group by [POLICY_NO]
		)a
		;
		...
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'DELETE DATA FROM FOND_ID.[FOND_ETL5_PRODUCTION_COMMISSION_DRIVER_DETAIL] : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		
		INSERT into FOND_ID.FOND_IFRS17_PROC_LOG(PROC_DATE,FUNC_NAME,SEQNO,DESCRIPTION)
		VALUES (@V_START,@V_FUNCTION_NAME,@V_SEQNO,@V_DESCRIPTION);	 

		DELETE FROM FOND_ID.FOND_ETL5_PRODUCTION_COMMISSION_DRIVER_DETAIL 
		WHERE DRIVER_PERIOD = YEAR(DATEADD(month, 0,CONVERT(date, @batch))) * 1000 + 0 + MONTH(DATEADD(month, 0,CONVERT(date, @batch)));

		 ---------------------------- TO Handle rerun process ------------------------------
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'INSERT INTO TABLE FOND_ID.[FOND_ETL5_PRODUCTION_COMMISSION_DRIVER_DETAIL] : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		INSERT into FOND_ID.FOND_IFRS17_PROC_LOG(PROC_DATE,FUNC_NAME,SEQNO,DESCRIPTION)
		VALUES (@V_START,@V_FUNCTION_NAME,@V_SEQNO,@V_DESCRIPTION);


		INSERT INTO [FOND_ID].[FOND_ETL5_PRODUCTION_COMMISSION_DRIVER_DETAIL]
		SELECT ENTITY_ID,
				DRIVER_SOURCE,
		...
		FROM #etl5_production_commission_driver
		;
		...
		SET @V_TOTAL_ROWS = (SELECT COUNT(1) as totalrows FROM #etl5_production_commission_driver) ;
		...
		VALUES (@V_START,@V_FUNCTION_NAME,'FOND_ID.FOND_ETL5_PRODUCTION_COMMISSION_DRIVER_DETAIL'
