CREATE PROC [FOND_ID].[USP_ETL5_SUNGL_COMPLETENESS_CHECK_SPLITTING] @batch [NVARCHAR](100),@JOBNAMESTR [NVARCHAR](2000) AS
BEGIN 
...
BEGIN TRY
		...
		(
		  SELECT
			BATCHDATE, ACCNT_CODE, ENTITY_ID, --DESCRIPTN,
		...
		LEFT JOIN
		  (
			...
			AND EXCL_FIELD1 = 'ACCT_CD' AND EXCL_FIELD1_COND IN('3141990000', '5119030000') AND ENTITY_ID IN ('IAC','IAS')
			GROUP BY EXCL_FIELD1_COND, ENTITY_ID
			UNION
			SELECT 
				EXCL_FIELD1_COND AS ACCNT_CODE, 'IAC' AS ENTITY_ID
			FROM FCORE_ID.FOND_EXCLUSION_CONFIG 
			WHERE RECONCILIATION_IND='Y' AND EXCL_FIELD1='ACCT_CD'			
			UNION
			SELECT 
				EXCL_FIELD1_COND AS ACCNT_CODE, 'IAS' AS ENTITY_ID
			FROM FCORE_ID.FOND_EXCLUSION_CONFIG 
			WHERE RECONCILIATION_IND='Y' AND EXCL_FIELD1='ACCT_CD'					
