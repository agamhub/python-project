CREATE PROC [FOND_ID].[USP_LOAD_ETL5_LIFEASIA_DRIVER_DETAIL_PREMIUM_FYP_COLLECTED_RENEWAL_PREMIUM] @batch [NVARCHAR](100) AS
BEGIN 
	--DECLARE @batch [nvarchar](30);
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'FOND_ID.USP_LOAD_ETL5_LIFEASIA_DRIVER_DETAIL_PREMIUM_FYP_COLLECTED_RENEWAL_PREMIUM';
...
BEGIN TRY
		...
		BEGIN
			DROP TABLE #etl5_las_Premium_FYP_Collected_Renewal_Premium_driver
		END;
...
--		) a';
		...
		SELECT 
			...
			(
				...
				union all
				select 
					SACSCODE, SACSTYP ,ACCT_PERIOD,TXN_DT,POLICY_NO,T0,AMT_RPT_CCY,LFAGLCODE,''Invalid'' AS TAG,PRODUCT_CD,BENEFIT_CD,EFFDATE,TRANS_CODE,SUN_CD,SUBSTRING(SUBSTRING(CAST(LFAGLCODE AS VARCHAR),1,6),6,1) NB_FLAG
				 from '+@V_TNAME_EB1+'
			 )A 
		...
		select
			...
			into #etl5_las_Premium_FYP_Collected_Renewal_Premium_driver --#FINAL
			from #acmv a
			left join #T0 f on a.PRODUCT_CD = f.PRODUCT_CD;
		...
		DELETE FROM FOND_ID.FOND_ETL5_LIFEASIA_DRIVER_DETAIL_PREMIUM_FYP_COLLECTED_RENEWAL_PREMIUM 
		WHERE ACCOUNTING_PERIOD =  YEAR(DATEADD(month, 0,CONVERT(date, @batch))) * 1000 + 0 + MONTH(DATEADD(month, 0,CONVERT(date, @batch)));

		 ---------------------------- TO Handle rerun process ------------------------------
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'INSERT INTO TABLE FOND_ID.[FOND_ETL5_LIFEASIA_DRIVER_DETAIL_PREMIUM_FYP_COLLECTED_RENEWAL_PREMIUM] : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		INSERT into FOND_ID.FOND_IFRS17_PROC_LOG(PROC_DATE,FUNC_NAME,SEQNO,DESCRIPTION)
		VALUES (@V_START,@V_FUNCTION_NAME,@V_SEQNO,@V_DESCRIPTION);


		INSERT INTO [FOND_ID].[FOND_ETL5_LIFEASIA_DRIVER_DETAIL_PREMIUM_FYP_COLLECTED_RENEWAL_PREMIUM]
		...
		FROM #etl5_las_Premium_FYP_Collected_Renewal_Premium_driver
		;
		...
		SET @V_TOTAL_ROWS = (SELECT COUNT(1) as totalrows FROM #etl5_las_Premium_FYP_Collected_Renewal_Premium_driver) ;
		...
		VALUES (@V_START,@V_FUNCTION_NAME,'FOND_ID.FOND_ETL5_LIFEASIA_DRIVER_DETAIL_PREMIUM_FYP_COLLECTED_RENEWAL_PREMIUM'
		...
		---------------------------- DROP TEMPORARY TABLE ------------------------------  
		IF OBJECT_ID('tempdb..#etl5_las_Premium_FYP_Collected_Renewal_Premium_driver') IS NOT NULL
		BEGIN
			DROP TABLE #etl5_las_Premium_FYP_Collected_Renewal_Premium_driver
		END;
