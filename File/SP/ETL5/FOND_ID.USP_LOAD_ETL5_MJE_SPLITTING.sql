CREATE PROC [FOND_ID].[USP_LOAD_ETL5_MJE_SPLITTING] @batch [NVARCHAR](100),@JOBNAMESTR [NVARCHAR](2000) AS
BEGIN 
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'FOND_ID.USP_LOAD_ETL5_MJE_SPLITTING';
...
--		declare @batch varchar(20)='20200701';
		...
		where a.BATCHDATE=left(@batch,6) and a.ENTITY_ID IN ('IAC','IAS')
		)
		,
		stepFiltered as 
		(
		...
		FROM #MJE where MJE_FLAG like '%Filtered%' and ENTITY_ID IN ('IAC','IAS')
		;
		...
		CONCAT(SUBSTRING(@batch,1,4), SUBSTRING(@batch,5,2)) AS BATCHDATE,
		CURRENT_TIMESTAMP AS ETL_PROCESS_DATE_TIME
		FROM #MJE where MJE_FLAG like '%Unfiltered%' and ENTITY_ID IN ('IAC','IAS')
		;
		---------------------------- ETL5 LOGGING ----------------------------      
		...
		SET @V_TOTAL_ROWS_FILTERED = (SELECT COUNT(1) as totalrows FROM FOND_ID.FOND_ETL5_MJE_FILTERED WHERE ENTITY_ID IN ('IAC','IAS')) ;
		SET @V_TOTAL_ROWS_UNFILTERED = (SELECT COUNT(1) as totalrows FROM FOND_ID.FOND_ETL5_MJE_UNFILTERED WHERE ENTITY_ID IN ('IAC','IAS')) ;
