CREATE PROC [FOND_ID].[USP_LOAD_ETL5_IFRS4_SUNGL] @batch [NVARCHAR](100),@JOBNAMESTR [NVARCHAR](2000) AS
BEGIN 
...
--					   WHEN A.SRC IN('NE_FOREX_4710300000', 'NE_RI') THEN CASE WHEN A.D_C = 'C' THEN 'D' ELSE 'C' END
			...
			SELECT DISTINCT *, 'IFRS4_SUNGL' AS SRC FROM STAG_ID.STAG_SUNGL_ABSTR_IFRS4_SUNGL WHERE replace(PERIOD,'/0','') =SUBSTRING(CAST(@batch AS VARCHAR),1,6 ) 
			
			UNION
			
			SELECT DISTINCT *, 'IFRS4_SUNGL_NEW_ENTRIES' AS SRC FROM [STAG_ID].[STAG_SUNGL_ABSTR_IFRS4_SUNGL_NEW_ENTRIES] WHERE replace(PERIOD,'/0','') =SUBSTRING(CAST(@batch AS VARCHAR),1,6 ) AND ENTITY_ID = 'IAI'
			
			UNION
			
			--NEW ENTRIES 9020000001
			SELECT DISTINCT *, 'IFRS4_SUNGL_9020000001' AS SRC FROM STAG_ID.STAG_SUNGL_ABSTR_IFRS4_SUNGL WHERE ACCNT_CODE = '4181010000' AND JRNAL_TYPE = 'UTR' AND replace(PERIOD,'/0','') =SUBSTRING(CAST(@batch AS VARCHAR),1,6 ) 
			
			UNION
			
			--NEW ENTRIES 9020000002
			SELECT DISTINCT *, 'IFRS4_SUNGL_9020000002' AS SRC FROM STAG_ID.STAG_SUNGL_ABSTR_IFRS4_SUNGL WHERE ACCNT_CODE = '4181010000' AND JRNAL_TYPE = 'UTR' AND replace(PERIOD,'/0','') =SUBSTRING(CAST(@batch AS VARCHAR),1,6 ) 
...
*/
            ...
            JOIN (SELECT JRNAL_NO, AMOUNT FROM STAG_ID.STAG_SUNGL_ABSTR_IFRS4_SUNGL WHERE ACCNT_CODE = '3392090080') AS SUNGL_3392090080
            ON SUNGL_3392090080.JRNAL_NO = GL_RI_PARTIAL_BIN.JRNAL_NO
            AND ABS(SUNGL.AMOUNT) = ABS(SUNGL_3392090080.AMOUNT)
            WHERE ABS(SUNGL.AMOUNT) IN(SELECT ABS(AMOUNT) FROM STAG_ID.STAG_SUNGL_ABSTR_IFRS4_SUNGL WHERE JRNAL_TYPE = 'REVS2' AND DESCRIPTN like 'NETOFF%' AND REPLACE    (PERIOD,'/0','') = SUBSTRING(CAST(@batch AS VARCHAR),1,6) GROUP BY ABS(AMOUNT) HAVING COUNT(*) > 1)
