CREATE PROC [FOND_ID].[USP_LOAD_ETL5_PER_ACCOUNT_VALUE_DRIVER_DETAIL_YTD] @batch [NVARCHAR](100),@JOBNAMESTR [NVARCHAR](2000) AS
BEGIN 
	--DECLARE @batch [nvarchar](30)=@batch1
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'FOND_ID.FOND_ETL5_PER_ACCOUNT_VALUE_DRIVER_DETAIL_YTD';
...
BEGIN TRY
		...
		SELECT 
				...
				[POLICY_NO] COLLATE SQL_Latin1_General_CP1_CS_AS as POL_NO ,
				...
				-- select top 10 *
		FROM
		[FOND_ID].[FOND_ETL5_LIFEASIA_DRIVER_DETAIL_PER_ACCOUNT_VALUE]
		...
		SELECT 
				...
				YEAR(DATEADD(month, 0,CONVERT(date, @batch))) * 1000 + 0 + MONTH(DATEADD(month, 0,CONVERT(date, @batch))) DRIVER_PERIOD,
				[POL_NO] COLLATE SQL_Latin1_General_CP1_CS_AS,
				max([BENF_CD]) COLLATE SQL_Latin1_General_CP1_CS_AS as BENF_CD,
				max([PROD_CD]) COLLATE SQL_Latin1_General_CP1_CS_AS as PROD_CD,
				'' [TREATY_CD],
				max([ADJ_T0]) COLLATE SQL_Latin1_General_CP1_CS_AS AS FUND,
