CREATE PROC [FOND_ID].[USP_LOAD_ETL5_SUNGL_ZERO_AUTOMATION_SPLITTING] @batch [nvarchar](30),@JOBNAMESTR [NVARCHAR](2000) AS

BEGIN 
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'USP_LOAD_ETL5_SUNGL_ZERO_AUTOMATION_SPLITTING';
...
--		declare @batch varchar(20)='202101';
		...
		ACCNT_CODE in (select distinct ACCNT_CODE from FOND_ID.ETL5_ETL4_INTEGRATION_CHECK where (PERIOD=left(@batch,4)+'/0'+right(@batch,2) or PERIOD_ETL4=left(@batch,4)+'0'+right(@batch,2)) and AMT_DIFF>=-1000 and AMT_DIFF<=1000 and STATUS='FAILED' and ENTITY_ID IN ('IAC','IAS'))
		--ACCNT_CODE in ('3373099990','1233010000')
		) n
		left join (select ACCNT_CODE,AMT_DIFF from FOND_ID.ETL5_ETL4_INTEGRATION_CHECK where (PERIOD=left(@batch,4)+'/0'+right(@batch,2)or PERIOD_ETL4=left(@batch,4)+'0'+right(@batch,2)) and AMT_DIFF>=-1000 and AMT_DIFF<=1000 and STATUS='FAILED' and ENTITY_ID IN ('IAC','IAS')) m on n.ACCNT_CODE=m.ACCNT_CODE
		...
		left join (select 'key' key_,sum(AMT_DIFF) AMT_DIFF from FOND_ID.ETL5_ETL4_INTEGRATION_CHECK where (PERIOD=left(@batch,4)+'/0'+right(@batch,2)or PERIOD_ETL4=left(@batch,4)+'0'+right(@batch,2)) and AMT_DIFF>=-1000 and AMT_DIFF<=1000 and STATUS='FAILED' and ENTITY_ID IN ('IAC','IAS')
		...
		declare @counter int=(select count(*) from FOND_ID.ETL5_ETL4_INTEGRATION_CHECK where (PERIOD=left(@batch,4)+'/0'+right(@batch,2) or PERIOD_ETL4=left(@batch,4)+'0'+right(@batch,2)) and STATUS='FAILED' and ENTITY_ID IN ('IAC','IAS') and AMT_DIFF between -1000 and 1000);
		if (@counter>0)
		delete from  STAG_ID.STAG_SUNGL_ABSTR_IFRS4_SUNGL_NEW_ENTRIES where BATCHDATE=left(@batch,6) and DESCRIPTN in ('ETL5-Zeroing recon balance','Accr Cash Back-ETL 5 Zeroist') and ENTITY_ID IN ('IAC','IAS'); 	
		else
		...
		select 'No rerun required for FOND_ID.USP_LOAD_ETL5_IFRS4_SUNGL_SPLITTING' ; 
	
		if (@counter>0)		
		exec FOND_ID.USP_LOAD_ETL5_SUNGL_EXTRACT_SPLITTING @batch,@JOBNAMESTR;
		else
		select 'No rerun required FOND_ID.USP_LOAD_ETL5_SUNGL_EXTRACT_SPLITTING' ; 

		if (@counter>0)		
		exec FOND_ID.USP_LOAD_ETL5_SUNGL_EXTRACT_EXCLUDED_SPLITTING @batch,@JOBNAMESTR;
		else
		select 'No rerun required FOND_ID.USP_LOAD_ETL5_SUNGL_EXTRACT_EXCLUDED_SPLITTING' ; 
	
		if (@counter>0)		
		exec FOND_ID.USP_LOAD_ETL5_SUNGL_EXTRACT_BINEXCLUSION_SEPARATION_SPLITTING @batch,@JOBNAMESTR;	
		else
		select 'No rerun required FOND_ID.USP_LOAD_ETL5_SUNGL_EXTRACT_BINEXCLUSION_SEPARATION_SPLITTING' ; 
