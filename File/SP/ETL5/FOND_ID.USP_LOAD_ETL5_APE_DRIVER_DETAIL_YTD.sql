CREATE PROC [FOND_ID].[USP_LOAD_ETL5_APE_DRIVER_DETAIL_YTD] @batch [NVARCHAR](100) AS
BEGIN 
	--DECLARE @batch [nvarchar](30);
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'FOND_ID.FOND_ETL5_APE_DRIVER_DETAIL';
...
BEGIN TRY
		...
		IF OBJECT_ID('tempdb..#etl5_ape_driver') IS NOT NULL
		BEGIN
			DROP TABLE #etl5_ape_driver
		END;

		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'SELECT INTO TEMPORARY TABLE etl5_ape_driver : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		...
		SELECT 
				...
				CAST([POLICY_NO] AS VARCHAR) as POL_NO,
				max([BENF_CD]) COLLATE SQL_Latin1_General_CP1_CI_AS as BENF_CD,
				max([PROD_CD]) COLLATE SQL_Latin1_General_CP1_CI_AS as PROD_CD,
				'' [TREATY_CD],
				max([ADJ_T0]) COLLATE SQL_Latin1_General_CP1_CI_AS AS FUND,
				@drivername ALLOCATION_DRIVER,
				sum(APE) DRIVER_AMOUNT,
				'0' BATCH_MASTER_ID,
				'0' BATCH_RUN_ID,
				'0' JOB_MASTER_ID,
				'0' JOB_RUN_ID,
		...
		[FOND_ID].[FOND_ETL5_LIFEASIA_DRIVER_DETAIL_APE]
		...
		SELECT 
				...
				max([ADJ_T0]) COLLATE SQL_Latin1_General_CP1_CI_AS AS FUND,
				@drivername ALLOCATION_DRIVER,
				sum(case when [APE] is null then 0 else APE end) as DRIVER_AMOUNT,
				'0' BATCH_MASTER_ID,
				'0' BATCH_RUN_ID,
				'0' JOB_MASTER_ID,
				'0' JOB_RUN_ID,
		...
		SELECT 
				...
				'' [TREATY_CD],
				 FUND COLLATE SQL_Latin1_General_CP1_CI_AS,
				@drivername ALLOCATION_DRIVER,
				sum(case when [AMOUNT] is null then 0 else AMOUNT*0.1 end) as DRIVER_AMOUNT,
				'0' BATCH_MASTER_ID,
				'0' BATCH_RUN_ID,
				'0' JOB_MASTER_ID,
				'0' JOB_RUN_ID,
				YEAR(DATEADD(month, 0,CONVERT(date, @batch))) * 100 + MONTH(DATEADD(month, 0,CONVERT(date, @batch))) DL_PLAI_BATCHDATE,
				GETDATE() ETL_PROCESS_DATE_TIME
		FROM (select *,'GTNN000' FUND,
		cast(left(REPLACE(convert(varchar,RECEIPT_DATE,112),'-',''),4)+'0'+substring(REPLACE(convert(varchar,RECEIPT_DATE,112),'-',''),5,2) as int) as ACCOUNTING_PERIOD
		from [FOND_ID].[FOND_ETL5_OMNI_DRIVER_DETAIL]) n
		...
		AND YEAR(DATEADD(month, 0,CONVERT(date,@batch))) * 1000 + 0 + MONTH(DATEADD(month, 0,CONVERT(date, @batch)))
		GROUP BY POL_NO,PRODUCT_CODE,FUND
		)a
		;
		...
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'DELETE DATA FROM FOND_ID.[FOND_ETL5_APE_DRIVER_DETAIL] : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		...
		DELETE FROM FOND_ID.FOND_ETL5_APE_DRIVER_DETAIL 
		WHERE DRIVER_PERIOD =  YEAR(DATEADD(month, 0,CONVERT(date, @batch))) * 1000 + 0 + MONTH(DATEADD(month, 0,CONVERT(date, @batch)));

		 ---------------------------- TO Handle rerun process ------------------------------
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'INSERT INTO TABLE FOND_ID.[FOND_ETL5_APE_DRIVER_DETAIL] : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		INSERT into FOND_ID.FOND_IFRS17_PROC_LOG(PROC_DATE,FUNC_NAME,SEQNO,DESCRIPTION)
		VALUES (@V_START,@V_FUNCTION_NAME,@V_SEQNO,@V_DESCRIPTION);


		INSERT INTO [FOND_ID].[FOND_ETL5_APE_DRIVER_DETAIL]
		(ENTITY_ID,
		DRIVER_SOURCE,
		DRIVER_PERIOD,
		POL_NO,
		...
		SET @V_TOTAL_ROWS = (SELECT COUNT(1) as totalrows FROM #etl5_ape_driver) ;
		...
		VALUES (@V_START,@V_FUNCTION_NAME,'FOND_ID.FOND_ETL5_APE_DRIVER_DETAIL'
