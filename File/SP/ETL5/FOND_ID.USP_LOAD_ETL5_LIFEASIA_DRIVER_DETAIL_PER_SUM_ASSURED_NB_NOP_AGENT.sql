CREATE PROC [FOND_ID].[USP_LOAD_ETL5_LIFEASIA_DRIVER_DETAIL_PER_SUM_ASSURED_NB_NOP_AGENT] @batch [NVARCHAR](100),@JOBNAMESTR [NVARCHAR](2000) AS
BEGIN 
	--DECLARE @batch [nvarchar](30);
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'FOND_ID.FOND_ETL5_LIFEASIA_DRIVER_DETAIL_PER_SUM_ASSURED_NB_NOP_AGENT';
	...
	BEGIN TRY
		...
		--) AND
		T0 not in ('-','0000LAS','ITYT000','0000000')
		...
		,fin as (
			...
			join hpad B on A.CHDRNUM = B.CHDRNUM 
			join covr C on B.CHDRNUM = C.CHDRNUM
			join (SELECT row_number () OVER (PARTITION BY PRODUCT_CD order by T0 desc) RN , * FROM T0_2) D on A.CNTTYPE = D.PRODUCT_CD AND D.RN = 1 --and substr(a.hoissdte,1,4)||'0'||substr(a.hoissdte,5,2) = d.acct_period
