CREATE PROC [FOND_ID].[USP_LOAD_ETL5_PAYABLE_ASSESSMENT_FINAL] @batch [NVARCHAR](10),@JOBNAMESTR [NVARCHAR](2000) AS 
BEGIN
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'FOND_ID.USP_LOAD_ETL5_PAYABLE_ASSESSMENT_FINAL';
...
BEGIN TRY
	...
	-- Get the unique list of ACCNT_CODE from SUNGL T+4, Exclude some ACCNT_CODE based on CONFIG and Get first digit of ACCT_CODE
	SET @V_DESCRIPTION = 'Get the unique list of ACCNT_CODE from SUNGL T+4 & Exclude some CoA: ' + convert(varchar,@V_START,121);
	PRINT @V_DESCRIPTION;
	SET @V_SEQNO = @V_SEQNO + 1;
	...
	-- Sorting based on ACCT_CODE and only get priority of REMARKS
	...
	FROM (
		SELECT ACCNT_CODE FROM (SELECT * FROM FOND_ID.FOND_ETL5_CONFIG_PAYABLE_ASSESSMENT_PRE WHERE BATCHDATE = LEFT(@batch,6)) A
		UNION 
	...
	LEFT JOIN (SELECT * FROM FOND_ID.FOND_ETL5_CONFIG_PAYABLE_ASSESSMENT_PRE WHERE BATCHDATE = LEFT(@batch,6)) BB ON AA.ACCNT_CODE = BB.ACCNT_CODE AND AA.PERIOD = BB.PERIOD
	...
	DELETE FROM [FOND_ID].[FOND_ETL5_CONFIG_PAYABLE_ASSESSMENT_FINAL] WHERE BATCHDATE=left(@batch,6);
	...
	-- Insert Data	
	INSERT INTO [FOND_ID].[FOND_ETL5_CONFIG_PAYABLE_ASSESSMENT_FINAL]
	SELECT * 
