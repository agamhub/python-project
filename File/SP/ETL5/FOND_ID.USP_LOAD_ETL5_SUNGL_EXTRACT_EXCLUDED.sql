CREATE PROC [FOND_ID].[USP_LOAD_ETL5_SUNGL_EXTRACT_EXCLUDED] @batch [NVARCHAR](100),@JOBNAMESTR [NVARCHAR](2000) AS
BEGIN 
...
--				WHERE LBL IS  NULL ) A 
		...
		select A.* 
			...
			from (
				...
				from  FOND_ID.FOND_ETL5_IFRS4_SUNGL where PERIOD=@V_DRIVER_PERIOD AND ENTITY_ID = 'IAI'
				) A
				LEFT JOIN (SELECT *,'ETL5' AS LBL FROM FOND_ID.FOND_ETL5_SUNGL_EXTRACT where EXTRACT_PERIOD=@V_DRIVER_PERIOD AND ENTITY_ID = 'IAI') B ON
				 A.JRNAL_NO=B.JRNAL_NO
				    AND A.JRNAL_LINE=B.JRNAL_LINE
		...
		DELETE FOND_ID.[FOND_PLAI_FOND_ETL5_SUNGL_EXCLUDED] where PERIOD=@V_DRIVER_PERIOD AND ENTITY_ID = 'IAI';
...
--		SELECT SUBSTRING(CAST(@batch AS VARCHAR),1,4)+'/0'+SUBSTRING(@batch,5,2)
		...
		SELECT 'IAI' as ENTITY_ID, LEDGER, ACCNT_CODE, PERIOD, TRANS_DATE, AMOUNT, D_C, ALLOCATION, JRNAL_NO, JRNAL_LINE, JRNAL_TYPE, JRNAL_SRCE, TREFERENCE, DESCRIPTN, ENTRY_DATE, ENTRY_PRD, DUE_DATE, ALLOC_REF, ALLOC_DATE, ALLOC_PERIOD, ASSET_IND, ASSET_CODE, ASSET_SUB, CONV_CODE, CONV_RATE, OTHER_AMT, OTHER_DP, CLEARDOWN, REVERSAL, LOSS_GAIN, ROUGH_FLAG, IN_USE_FLAG, ANAL_T0, ANAL_T1, ANAL_T2, ANAL_T3, ANAL_T4, ANAL_T5, ANAL_T6, ANAL_T7, ANAL_T8, ANAL_T9, POSTING_DATE, ALLOC_IN_PROGRESS, HOLD_REF, HOLD_OP_ID, LAST_CHANGE_USER_ID, LAST_CHANGE_DATE, ORIGINATOR_ID, UPLOAD_DATE, FNAME, FPROCDATE, SOURCE_TYPE, PROCESS_TYPE, PROCESS_TIME, JOB_PROCESS_ID, BATCH_MASTER_ID, BATCH_RUN_ID, JOB_MASTER_ID, JOB_RUN_ID,left(@batch,6) as BATCHDATE, ETL_PROCESS_DATE_TIME
