CREATE PROC [FOND_ID].[USP_LOAD_ETL5_BIN_ACCOUNT_CONFIG] @batch [NVARCHAR](100) AS
BEGIN 
	--DECLARE @batch [nvarchar](30);
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'FOND_ID.FOND_ETL5_BIN_ACCOUNT_CONFIG';
...
BEGIN TRY
		...
		IF OBJECT_ID('tempdb..#BIN_CONFIG') IS NOT NULL
		BEGIN
			DROP TABLE #BIN_CONFIG
		END;

		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'SELECT INTO TEMPORARY TABLE BIN_CONFIG : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		...
		SELECT CAST(ENTITY_ID AS varchar(5)) AS ENTITY_ID
			,CAST(EXCL_FIELD1 AS varchar(15)) AS EXCL_FIELD1
			,CAST(EXCL_FIELD1_COND AS varchar(15)) AS EXCL_FIELD1_COND
			,CAST(EXCL_FIELD2 AS varchar(15)) AS EXCL_FIELD2
			,CAST(EXCL_FIELD2_COND AS varchar(15)) AS EXCL_FIELD2_COND
			...
			INTO #BIN_CONFIG
			FROM
				STAG_ID.STAG_BIN_ACCOUNT_CONFIG_DQ_DDLREF_EXTTB
				;
		...
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'DELETE DATA FROM FOND_ID.[FOND_ETL5_BIN_ACCOUNT_CONFIG] : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		
		INSERT into FOND_ID.FOND_IFRS17_PROC_LOG(PROC_DATE,FUNC_NAME,SEQNO,DESCRIPTION)
		VALUES (@V_START,@V_FUNCTION_NAME,@V_SEQNO,@V_DESCRIPTION);	 

		-- SELECT DISTINCT DRIVER_PERIOD FROM FOND_ID.FOND_ETL5_BIN_ACCOUNT_CONFIG
	
		DELETE FROM FOND_ID.FOND_ETL5_BIN_ACCOUNT_CONFIG;
		--WHERE ACCOUNTING_PERIOD =  YEAR(DATEADD(month, 0,CONVERT(date, @batch))) * 1000 + 0 + MONTH(DATEADD(month, 0,CONVERT(date, @batch)));

		 ---------------------------- TO Handle rerun process ------------------------------
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'INSERT INTO TABLE FOND_ID.[FOND_ETL5_BIN_ACCOUNT_CONFIG] : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		INSERT into FOND_ID.FOND_IFRS17_PROC_LOG(PROC_DATE,FUNC_NAME,SEQNO,DESCRIPTION)
		VALUES (@V_START,@V_FUNCTION_NAME,@V_SEQNO,@V_DESCRIPTION);
		...
		(ENTITY_ID, EXCL_FIELD1, EXCL_FIELD1_COND, EXCL_FIELD2, EXCL_FIELD2_COND, BIN_FLG, BIN_EXCL_SUMMARY_DESC, RULE_ID, [ACTION], USER_PROFILE, UPDATE_DATE, BATCH_MASTER_ID, BATCH_RUN_ID, JOB_MASTER_ID, JOB_RUN_ID, BATCHDATE, ETL_PROCESS_DATE_TIME)
		SELECT 
		ENTITY_ID, EXCL_FIELD1, EXCL_FIELD1_COND, EXCL_FIELD2, EXCL_FIELD2_COND, EXCL_FLAG, DESCRIPTION, RULE_ID, [ACTION], USER_PROFILE, UPDATE_DATE, BATCH_MASTER_ID, BATCH_RUN_ID, JOB_MASTER_ID, JOB_RUN_ID, BATCHDATE, ETL_PROCESS_DATE_TIME
		FROM #BIN_CONFIG
		;
		...
		SET @V_TOTAL_ROWS = (SELECT COUNT(1) as totalrows FROM #BIN_CONFIG) ;
		...
		VALUES (@V_START,@V_FUNCTION_NAME,'FOND_ID.FOND_ETL5_BIN_ACCOUNT_CONFIG'
