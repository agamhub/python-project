CREATE PROC [FOND_ID].[USP_LOAD_ETL5_NOP_APE_DRIVER_DETAIL_TESTING] @batch [nvarchar](30) AS

BEGIN 
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'FOND_ID.FOND_MIX_APE_NOV_DRIVER_DETAIL';
	...
	FROM (
				 SELECT a.*,cast(CAST(DRIVER_RATIO AS numeric(28,6))/@V_SUM_APE as numeric(28,6)) AS WEIGHT FROM #driver_ratio a WHERE SINGLE_DRIVER_CODE='APE'
				 union
				 SELECT a.*,cast(CAST(DRIVER_RATIO AS numeric(28,6))/@V_SUM_NOP as numeric(28,6)) AS WEIGHT FROM #driver_ratio a WHERE SINGLE_DRIVER_CODE='NOP'
	)a; 
...
--	);
	;	
 	--------------------------------------------------------------------------------
 	DELETE FROM  [FOND_ID].[FOND_ETL5_DRIVER_DETAIL_MIX_TESTING]
			WHERE [DRIVER_PERIOD] = YEAR(DATEADD(month, 0,CONVERT(date, @batch))) * 1000 + 0 + MONTH(DATEADD(month, 0,CONVERT(date, @batch)))
			AND ALLOCATION_DRIVER='APE_NOP';
		
	INSERT INTO [FOND_ID].[FOND_ETL5_DRIVER_DETAIL_MIX_TESTING]
