CREATE PROC [FOND_ID].[USP_LOAD_ETL5_NOP_NB_APE_DRIVER_DETAIL] @batch [nvarchar](30) AS

BEGIN 
	DECLARE @V_START		datetime;
	DECLARE @V_END			datetime;
	DECLARE @V_FUNCTION_NAME	NVARCHAR(2000) = 'FOND_ID.FOND_ETL5_MIX_NOP_NB_APE_DRIVER_DETAIL';
	...
	DECLARE @V_SUM_NOP_NB numeric(28,6);
	DECLARE @V_SUM_ALL numeric(28,6);
	DECLARE @V_PERCENTAGE_APE integer = 0;
	DECLARE @V_PERCENTAGE_NOP_NB integer = 0;;
	...
	IF OBJECT_ID('tempdb..#driver_ratio_nop_nb_ape') IS NOT NULL
	BEGIN
		DROP TABLE #driver_ratio_nop_nb_ape
	END;
		
	IF OBJECT_ID('tempdb..#driver_ratio_and_allocation_nop_nb_ape') IS NOT NULL
	BEGIN
		DROP TABLE #driver_ratio_and_allocation_nop_nb_ape
	END;
	
	IF OBJECT_ID('tempdb..#driver_sum_table_mixed_driver_nop_nb_ape') IS NOT NULL
	BEGIN
		DROP TABLE #driver_sum_table_mixed_driver_nop_nb_ape
	END;
	 
	IF OBJECT_ID('tempdb..#driver_detail_mix_nop_nb_ape') IS NOT NULL
	BEGIN
		DROP TABLE #driver_detail_mix_nop_nb_ape
	END; 
	...
	SELECT	
		...
		@V_PERCENTAGE_NOP_NB = B.PERCENTAGE
	FROM 
		(SELECT * FROM STAG_ID.STAG_CONFIG_MIX_DRIVER_DETAIL WHERE SINGLE_DRIVER_CODE='APE' and MIX_DRIVER_CODE='NOPNB_APE' ) A INNER JOIN 
		(SELECT * FROM STAG_ID.STAG_CONFIG_MIX_DRIVER_DETAIL WHERE SINGLE_DRIVER_CODE='NOP_NB' and MIX_DRIVER_CODE='NOPNB_APE' ) B
	...
	SELECT * 
	INTO 
		#driver_ratio_nop_nb_ape
	FROM (
		SELECT 
			DRIVER_PERIOD COLLATE DATABASE_DEFAULT as DRIVER_PERIOD,
			'NOPNB_APE' +  SUBSTRING(@batch,0,7) COLLATE DATABASE_DEFAULT AS ALLOCATION_DRIVER,
			POL_NO COLLATE DATABASE_DEFAULT as POL_NO,
			'NOPNB_APE' COLLATE DATABASE_DEFAULT as MIX_DRIVER_CODE,
		...
		SELECT 
			...
			'NOPNB_APE' +  SUBSTRING(@batch,0,7) COLLATE DATABASE_DEFAULT AS ALLOCATION_DRIVER,
			POL_NO COLLATE DATABASE_DEFAULT as POL_NO,
			'NOPNB_APE' COLLATE DATABASE_DEFAULT MIX_DRIVER_CODE,
			'NOP_NB' COLLATE DATABASE_DEFAULT SINGLE_DRIVER_CODE,
			DRIVER_AMOUNT AS DRIVER_AMOUNT,
			(DRIVER_AMOUNT * (@V_PERCENTAGE_NOP_NB * 0.01)) AS DRIVER_RATIO
		FROM
			(	
				SELECT 
					DRIVER_PERIOD,POL_NO,SUM(DRIVER_AMOUNT) DRIVER_AMOUNT
				FROM 
					[FOND_ID].[FOND_ETL5_NOP_NB_DRIVER_DETAIL]
				WHERE 
					DRIVER_PERIOD = YEAR(DATEADD(month, 0,CONVERT(date, @batch))) * 1000 + 0 + MONTH(DATEADD(month, 0,CONVERT(date, @batch)))
				GROUP BY 
					DRIVER_PERIOD,POL_NO
	...
	SELECT @V_SUM_APE = SUM(CAST(DRIVER_RATIO AS numeric(28,6))) FROM #driver_ratio_nop_nb_ape WHERE SINGLE_DRIVER_CODE='APE';
	SELECT @V_SUM_NOP_NB =  SUM(CAST(DRIVER_RATIO AS numeric(28,6)))  FROM #driver_ratio_nop_nb_ape WHERE SINGLE_DRIVER_CODE='NOP_NB';
	...
	SELECT
				 *
	INTO
				 #driver_ratio_and_allocation_nop_nb_ape 
	FROM (
				 SELECT a.*,cast(COALESCE(CAST(DRIVER_RATIO AS numeric(28,6))/ NULLIF(@V_SUM_APE,0), 0) as numeric(28,6)) AS WEIGHT FROM #driver_ratio_nop_nb_ape a WHERE SINGLE_DRIVER_CODE='APE'
				 union
				 SELECT a.*,cast(COALESCE(CAST(DRIVER_RATIO AS numeric(28,6))/NULLIF(@V_SUM_NOP_NB,0), 0) as numeric(28,6)) AS WEIGHT FROM #driver_ratio_nop_nb_ape a WHERE SINGLE_DRIVER_CODE='NOP_NB'
	)a; 
	...
	INTO 
		#driver_sum_table_mixed_driver_nop_nb_ape
	FROM 
		#driver_ratio_and_allocation_nop_nb_ape 
	GROUP BY 
		DRIVER_PERIOD,
	...
	SELECT 
		a.*,case when @V_SUM_ALL=0 then 0 else (COALESCE(CAST(b.POLICY_AMOUNT_SUM AS numeric(28,6))/ NULLIF(CAST(@V_SUM_ALL AS numeric(28,6)),0), 0)) end AS DRIVER_AMOUNT 
	INTO	
		#driver_detail_mix_nop_nb_ape
	FROM (
		SELECT 
			DISTINCT 
			*
	...
	)a inner join #driver_sum_table_mixed_driver_nop_nb_ape b 
	...
	DELETE FROM FOND_ID.[FOND_ETL5_DRIVER_RATIO_AND_ALLOCATION]
			...
			AND MIX_DRIVER_CODE='NOPNB_APE';
	
	INSERT INTO FOND_ID.FOND_ETL5_DRIVER_RATIO_AND_ALLOCATION
	SELECT DRIVER_PERIOD,ALLOCATION_DRIVER,POL_NO,MIX_DRIVER_CODE,SINGLE_DRIVER_CODE,DRIVER_AMOUNT,DRIVER_RATIO,WEIGHT 
	FROM #driver_ratio_and_allocation_nop_nb_ape;
...
--	);
	...
	INSERT INTO [FOND_ID].[FOND_ETL5_DRIVER_DETAIL_TOTAL_RATIO_POLICY] 
			SELECT * FROM #driver_sum_table_mixed_driver_nop_nb_ape;
...
--	);
	;	
 	...
 	DELETE FROM  [FOND_ID].[FOND_ETL5_DRIVER_DETAIL_MIX]
			WHERE [DRIVER_PERIOD] = YEAR(DATEADD(month, 0,CONVERT(date, @batch))) * 1000 + 0 + MONTH(DATEADD(month, 0,CONVERT(date, @batch)))
			AND ALLOCATION_DRIVER='NOPNB_APE';
	...
	INSERT INTO [FOND_ID].[FOND_ETL5_DRIVER_DETAIL_MIX]
			SELECT *,'0' BATCH_MASTER_ID,
				...
				GETDATE() as ETL_PROCESS_DATE_TIME FROM #driver_detail_mix_nop_nb_ape; 
