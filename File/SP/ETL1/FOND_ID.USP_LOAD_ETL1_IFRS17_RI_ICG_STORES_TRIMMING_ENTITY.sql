CREATE PROC [FOND_ID].[USP_LOAD_ETL1_IFRS17_RI_ICG_STORES_TRIMMING_ENTITY] @BATCHDATESTR [NVARCHAR](10),@JOBNAMESTR [NVARCHAR](2000),@ENTITY [NVARCHAR](2000),@PRD_ID [VARCHAR](10) AS
...
--BEGIN TRY
	...
	DECLARE @V_START			DATETIME,
			...
			@V_SEQNO			INTEGER,
	        @V_PRD_ID			VARCHAR(10),
			@V_CREATED_DATE		VARCHAR(10),
			@V_START_DATE		VARCHAR(10),
	        @V_END_DATE			VARCHAR(10),
			@PRUTOP_DATE		NVARCHAR(8),
			@ENTITY_FLG			NVARCHAR(20);

		DECLARE @v_SQL_MSTR NVARCHAR(MAX), @v_SQL NVARCHAR(MAX), @v_SQL1 NVARCHAR(MAX), @v_SQL2 NVARCHAR(MAX),  @v_SQL3 NVARCHAR(MAX)
,  @v_SQL4 NVARCHAR(MAX),  @v_SQL5 NVARCHAR(MAX),  @v_SQL6 NVARCHAR(MAX),  @v_SQL7 NVARCHAR(MAX)

	--SET VARIABLE
	SET @V_FUNCTION_NAME = 'FOND_ID.USP_LOAD_ETL1_IFRS17_RI_ICG_STORES_TRIMMING_ENTITY';
	SET @V_SEQNO = 1;
	...
	--drop permanent object if exists
	 IF OBJECT_ID('STAG_ID.STAG_TEMP_ETL1_RS_RIDER_RCD_RI_TRIMMING','U') IS NOT NULL EXEC('DROP TABLE STAG_ID.STAG_TEMP_ETL1_RS_RIDER_RCD_RI_TRIMMING');

	
    --IF OBJECT_ID('STAG_ID.STAG_TEMP_ETL1_RS_RIDER_RCD_RI_TRIMMING_'+@ENTITY+'','U') IS NOT NULL EXEC('DROP TABLE STAG_ID.STAG_TEMP_ETL1_RS_RIDER_RCD_RI_TRIMMING_'+@ENTITY+'');
	IF OBJECT_ID('STAG_ID.STAG_TEMP_ETL1_RS_RI_ICG_STORE_VALIDFLAG_'+@ENTITY+'','U') IS NOT NULL EXEC('DROP TABLE STAG_ID.STAG_TEMP_ETL1_RS_RI_ICG_STORE_VALIDFLAG_'+@ENTITY+'');
	IF OBJECT_ID('FOND_ID.FOND_RI_ICG_STORES_TRIMMED_'+@ENTITY+'','U') IS NOT NULL EXEC('DROP TABLE FOND_ID.FOND_RI_ICG_STORES_TRIMMED_'+@ENTITY+'');
	IF OBJECT_ID('FOND_ID.FOND_RI_ICG_STORES_TRIMMED_INVALID_'+@ENTITY+'','U') IS NOT NULL EXEC('DROP TABLE FOND_ID.FOND_RI_ICG_STORES_TRIMMED_INVALID_'+@ENTITY+'');
...
SELECT [PRODUCT_CD]
      ...
      ,[COUNTERPARTY_ID]
FROM ABST_ID'+@ENTITY_FLG+'.ABST_RI_PROD_TREATY_MAP_MASTER --ABST_ID.ABST_RI_PROD_TREATY_MAP_MASTER
...
SELECT RI_ICG_STORE.[ENTITY_ID]
	  ...
	  INTO STAG_ID.STAG_TEMP_ETL1_RS_RI_ICG_STORE_VALIDFLAG_'+@ENTITY_FLG+'
FROM
(( 
SELECT [ENTITY_ID]
      ,[POLICY_NO]
...
FROM
	...
	FROM --FCORE_ID.FOND_RI_ICG_STORE_UC AS ICG_STORE --RI ICG STORE FROM RHO
		  ----- START EDITED BY Dyah 221201---
				...
				FROM FCORE_ID'+@ENTITY_FLG+'.FOND_RI_ICG_STORE_UC ) AS ICG_STORE
				----- END EDITED BY Dyah----
    LEFT JOIN (SELECT 
 A.[PRODUCT_CD]
,A.[BENEFIT_CD]
...
SET @v_SQL_MSTR = CONCAT(@v_SQL1,@v_SQL2,@v_SQL3,@v_SQL4,@v_SQL5)

SET @v_SQL = REPLACE(@v_SQL_MSTR,'ENTITY_FLG',@ENTITY)
EXEC (@v_SQL)
...
INTO FOND_ID.FOND_RI_ICG_STORES_TRIMMED_'+@ENTITY_FLG+'	 --FINAL OUTPUT THAT NEED TO BE EXPORTED FOR ETL2B PURPOSE
FROM FCORE_ID'+@ENTITY_FLG+'.FOND_RI_ICG_STORE_UC AS ICG_STORE --RI ICG STORE FROM RHO
LEFT JOIN STAG_ID.STAG_TEMP_ETL1_RS_RI_ICG_STORE_VALIDFLAG_'+@ENTITY_FLG+' AS VALID_FLAG --INTO POINT 2
...
WHERE ICG_STORE.[RI_ICG_ID] NOT LIKE ''%XL%''
'
...
SELECT [ENTITY_ID]
	  ...
	  ,''INVALID UC_RI_CB_START_DT > UC_RI_CB_END_DT'' AS [DESC],
		...
		'+@BATCHDATESTR+' AS BATCHDATE,
		'''+@GMT_START_DTTM+''' AS ETL_PROCESS_DATE_TIME
	  INTO FOND_ID.FOND_RI_ICG_STORES_TRIMMED_INVALID_'+@ENTITY_FLG+'
FROM FCORE_ID'+@ENTITY_FLG+'.FOND_RI_ICG_STORE_UC
...
SET @v_SQL_MSTR ='
INSERT INTO FOND_ID.FOND_RI_ICG_STORES_TRIMMED_INVALID_'+@ENTITY_FLG+'
SELECT [ENTITY_ID]
      ,[POLICY_NO]
      ,[PRODUCT_CD]
      ,[BENEFIT_CD]
...
(SELECT ICG_STORE.*, (CASE WHEN (CASE WHEN ICG_STORE.[CB_END_DT] <= REF.[TREATY_END_DT] THEN ICG_STORE.[CB_END_DT] ELSE REF.[TREATY_END_DT]END) = ICG_STORE.[UC_RI_CB_END_DT] 
  ...
  FROM FCORE_ID'+@ENTITY_FLG+'.FOND_RI_ICG_STORE_UC AS ICG_STORE 
  LEFT JOIN (SELECT [PRODUCT_CD], [BENEFIT_CD], [TREATY_ID], [TREATY_END_DT] FROM ABST_ID'+@ENTITY_FLG+'.ABST_RI_PROD_TREATY_MAP_MASTER) AS REF
...
SELECT [ENTITY_ID]
	  ...
	  ,''INVALID RI_ICG_ID'' AS [DESC],
		...
		'''+@GMT_START_DTTM+''' AS ETL_PROCESS_DATE_TIME
FROM
(SELECT RI.* ,CASE WHEN CONCAT('''+@ENTITY_FLG+''', REF.[RI_PORTFOLIO_GROUP], ''_'', (CASE WHEN MAP_RN.[PRODUCT_CD] IS NOT NULL THEN RIGHT(RI.[RI_ICG_ID], 4) ELSE (CASE WHEN RI.[COHORT_YEAR] >= YEAR(REF.[TREATY_EFF_DT]) THEN RI.[COHORT_YEAR] ELSE YEAR(REF.[TREATY_EFF_DT]) END) END)) = CONCAT('''+@ENTITY_FLG+''', REF.[RI_PORTFOLIO_GROUP], ''_'', (CASE WHEN MAP_RN.[PRODUCT_CD] IS NOT NULL THEN (CASE WHEN RIGHT(MAP_RN.[RI_ICG_ID], 4) < ''2021'' THEN ''2021'' ELSE RIGHT(MAP_RN.[RI_ICG_ID], 4) END) ELSE (CASE WHEN RI.[COHORT_YEAR] >= YEAR(REF.[TREATY_EFF_DT]) THEN RI.[COHORT_YEAR] ELSE YEAR(REF.[TREATY_EFF_DT]) END) END)) THEN 1 ELSE 0 END AS [DEFECT_IND] 
  FROM FCORE_ID'+@ENTITY_FLG+'.FOND_RI_ICG_STORE_UC AS RI 
  LEFT JOIN (SELECT 
			[PRODUCT_CD],
			[BENEFIT_CD],
			[TREATY_ID],
			[TREATY_EFF_DT],
			[RI_PORTFOLIO_GROUP] 
			FROM ABST_ID'+@ENTITY_FLG+'.ABST_RI_PROD_TREATY_MAP_MASTER) AS REF
	...
	LEFT JOIN (
		...
		FROM
			...
			ORDER BY
				...
				4) DESC) AS [RN]
			FROM
				FCORE_ID'+@ENTITY_FLG+'.FOND_RI_UNDERLYING_ICG_MAP) AS MAP
		WHERE
