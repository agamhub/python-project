CREATE PROC [FOND_ID].[USP_LOAD_ETL1_IFRS17_FNC_MONTHLY_ICG_MPFILES_PRUEMAS] @BATCHDATESTR [NVARCHAR](10),@JOBNAMESTR [NVARCHAR](2000),@PRD_ID [INTEGER] AS
BEGIN	
	BEGIN TRY
			--declare variable
...
--------set variable----------------
	SET @V_FUNCTION_NAME = 'FOND_ID.USP_LOAD_ETL1_IFRS17_FNC_MONTHLY_ICG_MPFILES_PRUEMAS';
	SET @V_SEQNO = 1;
...
-----------------------------
	...
	IF OBJECT_ID('FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_DUP') IS NOT NULL DROP TABLE FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_DUP ;
	
	IF OBJECT_ID('STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS_ICG_FLAG') IS NOT NULL DROP TABLE STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS_ICG_FLAG;
	
	IF OBJECT_ID('STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS') IS NOT NULL DROP TABLE STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS;
	
	IF OBJECT_ID('tempdb.dbo.#TMP_IFRS17_ICG_STORES_PREMAS_TMP_1', 'U') IS NOT NULL DROP TABLE tempdb.dbo.#TMP_IFRS17_ICG_STORES_PREMAS_TMP_1;
	
	--IF OBJECT_ID('FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_INVALID') IS NOT NULL DROP TABLE FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_INVALID;
	TRUNCATE TABLE FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_INVALID;
	TRUNCATE TABLE FOND_ID.FOND_IFRS17_MPFILES_PRUEMAS ;
		...
		SET @V_DESCRIPTION	= 'CREATE TABLE FOND_IFRS17_PRUEMAS_DUP  : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		...
		-- check duplicate
		--drop table if exists FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_DUP ;
		/*
		IF OBJECT_ID('FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_DUP') IS NOT NULL DROP TABLE FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_DUP ;
		*/

		CREATE TABLE FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_DUP  
		WITH
		(
      ...
      @GMT_START_DTTM AS ETL_PROCESS_DATE_TIME
		from STAG_ID.STAG_PRUEMAS_STAG_ETL1_POLICY_RELATED
		where POLICY_ID in (select POLICY_ID from STAG_ID.STAG_PRUEMAS_STAG_ETL1_POLICY_RELATED  
		...
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'CREATE TABLE TMP_IFRS17_ICG_STORES_PRUEMAS_ICG_FLAG : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		...
		--drop table if exists STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS_icg_flag;
		/*
		IF OBJECT_ID('STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS_ICG_FLAG') IS NOT NULL DROP TABLE STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS_ICG_FLAG;
		*/

		CREATE TABLE STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS_ICG_FLAG 
		WITH
		(
		...
		AS 
		select distinct (CASE WHEN NULLIF(TRIM(A.POLICY_ID),'') IS NOT NULL THEN A.POLICY_ID ELSE 'N/A' END) AS POLICY_NO
	  ...
	  @BATCHDATESTR AS BATCHDATE,
      @GMT_START_DTTM AS ETL_PROCESS_DATE_TIME
		FROM STAG_ID.STAG_PRUEMAS_STAG_ETL1_POLICY_RELATED A
		LEFT JOIN (
			select a.*
			...
			where lower(trim(SUBSYSTEM)) IN (lower(trim('PruEmas')),lower(trim('PruEmasSya')))
			AND PRODUCT_CD = BENEFIT_CD
		) ICGS
		ON UPPER(TRIM(A.POLICY_ID)) = UPPER(TRIM(ICGS.POLICY_NO)) AND ICGS.idx = 1
		LEFT JOIN FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_DUP dup on UPPER(TRIM(A.POLICY_ID)) = UPPER(TRIM(dup.POLICY_ID))
		LEFT JOIN STAG_ID.STAG_CONFIG_IFRS17_CB_MAPPING CBM ON LOWER(TRIM(A.PRODCD)) = LOWER(TRIM(CBM.PRODUCT_CD))
		AND UPPER(TRIM(CBM.SYSTEM)) in (UPPER(TRIM(CAST('PruEmas' AS VARCHAR(20)))),UPPER(TRIM(CAST('PruEmasSya' AS VARCHAR(20)))))
		WHERE dup.POLICY_ID IS NULL
		;
	
		
				----------------------------------------------
		--create temp table tempdb.dbo.#TMP_IFRS17_ICG_STORES_PREMAS_TMP_1
		-----------------------------------------------
		
		SET @V_SEQNO = @V_SEQNO + 1;
		SET @V_START = CURRENT_TIMESTAMP;
		SET @V_DESCRIPTION = CONCAT('create temp table tempdb.dbo.#TMP_IFRS17_ICG_STORES_PREMAS_TMP_1 : ', @V_START);

		INSERT INTO STAG_ID.STAG_IFRS17_PROC_LOG(PROC_DATE, FUNC_NAME, SEQNO, DESCRIPTION)
		VALUES (@V_START, @V_FUNCTION_NAME, @V_SEQNO, @V_DESCRIPTION);

		
		CREATE TABLE tempdb.dbo.#TMP_IFRS17_ICG_STORES_PREMAS_TMP_1
		WITH
		(
		  DISTRIBUTION = HASH(POLICY_NO),
		  HEAP
		...
		,(CASE WHEN A.PRODCD = 'PruEmasS' then 'PruEmasSya'
			ELSE 'PruEmas' end) AS SYSTEM
		...
		,(CASE WHEN ICG_FLAG = 'NEW BUSINESS' THEN
					(CASE 
						WHEN BOUND_BY_REPRICE='N' AND BOUND_BY_TERMINATION_PROVISION = 'N' THEN convert(date,FORMAT(@V_START_DATE,'yyyy') + '1231')
		...
		--,CAST(A.CURRENCY AS VARCHAR(3)) AS CCY_CD
		...
		FROM STAG_ID.STAG_PRUEMAS_STAG_ETL1_POLICY_RELATED A
		LEFT JOIN FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_DUP dup on UPPER(TRIM(A.POLICY_ID)) = UPPER(TRIM(dup.POLICY_ID))
		LEFT JOIN STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS_ICG_FLAG icg_flag 
		on (CASE WHEN NULLIF(TRIM(A.POLICY_ID),'') IS NOT NULL THEN UPPER(TRIM(A.POLICY_ID)) ELSE 'N/A' END) = UPPER(TRIM(icg_flag.POLICY_NO))
		LEFT JOIN (
			select a.*
			, row_number() over(partition by POLICY_NO 
			order by CB_START_DT desc, CB_END_DT desc) as idx 
		...
		AND UPPER(TRIM(PFM.SYSTEM)) in (UPPER(TRIM(CAST('PruEmas' AS VARCHAR(20)))),UPPER(TRIM(CAST('PruEmasSya' AS VARCHAR(20)))))
		...
		AND UPPER(TRIM(SC.SYSTEM)) in (UPPER(TRIM(CAST('PruEmas' AS VARCHAR(20)))),UPPER(TRIM(CAST('PruEmasSya' AS VARCHAR(20)))))
		...
		AND UPPER(TRIM(SMT.SYSTEM)) in (UPPER(TRIM(CAST('PruEmas' AS VARCHAR(20)))),UPPER(TRIM(CAST('PruEmasSya' AS VARCHAR(20)))))
		WHERE dup.POLICY_ID is null
		;
		...
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'CREATE TABLE ifrs17_icg_stores_PRUEMAS : ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
	
		INSERT into STAG_ID.STAG_IFRS17_PROC_LOG(PROC_DATE,FUNC_NAME,SEQNO,DESCRIPTION)
		VALUES (@V_START,@V_FUNCTION_NAME,@V_SEQNO,@V_DESCRIPTION);

		CREATE TABLE STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS 
		WITH
		(
		 DISTRIBUTION = HASH(POLICY_NO)
		 ,CLUSTERED INDEX(POLICY_NO)
		...
		FROM tempdb.dbo.#TMP_IFRS17_ICG_STORES_PREMAS_TMP_1;
		...
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'CREATE VIEW  FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_INVALID: ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
	
		INSERT into STAG_ID.STAG_IFRS17_PROC_LOG(PROC_DATE,FUNC_NAME,SEQNO,DESCRIPTION)
		VALUES (@V_START,@V_FUNCTION_NAME,@V_SEQNO,@V_DESCRIPTION);

		--drop table if exists FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_INVALID;
		INSERT INTO FOND_ID.FOND_IFRS17_ICG_STORES_PRUEMAS_INVALID
		select 
		 a.ENTITY_ID
		,a.SYSTEM
		,a.POLICY_NO
		...
		,@BATCHDATESTR AS BATCHDATE
		,@GMT_START_DTTM AS ETL_PROCESS_DATE_TIME
		from STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS a
		where 
		(case 	
		...
		from STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS 
		where UPPER(TRIM(FOND_IFRS17_ICG_STORES.POLICY_NO)) = UPPER(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.POLICY_NO))
		AND TMP_IFRS17_ICG_STORES_PRUEMAS.ICG_FLAG = 'NEW BUSINESS'
		AND lower(trim(FOND_IFRS17_ICG_STORES.SUBSYSTEM)) in (lower(trim('PruEmas')),lower(trim('PruEmasSya')))
		AND (case 	
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.POLICY_NO),'') is null then 'N'
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.PRODUCT_CD),'') is null then 'N'
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.BENEFIT_CD),'') is null then 'N'
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.CONTRACT_ID),'') is null then 'N'
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.PORTFOLIO_GROUP),'') is null then 'N'
			when TMP_IFRS17_ICG_STORES_PRUEMAS.COHORT_YEAR is null then 'N'
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.SET_OF_CONTRACT),'') is null then 'N'
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.INSURANCE_CONTRACT_GROUP_ID),'') is null then 'N'
			when TMP_IFRS17_ICG_STORES_PRUEMAS.ICG_ID_PROPHET is null then 'N'
			when TMP_IFRS17_ICG_STORES_PRUEMAS.ENTRY_MONTH is null then 'N'
			when TMP_IFRS17_ICG_STORES_PRUEMAS.CB_START_DT is null then 'N'
			when TMP_IFRS17_ICG_STORES_PRUEMAS.CB_END_DT is null then 'N'
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.MEASUREMENT_MODEL),'') is null then 'N'
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.INFORCE_FLAG),'') is null then 'N'
			when TMP_IFRS17_ICG_STORES_PRUEMAS.COVERAGE_LEVEL_END_DATE is null then 'N'
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.CCY_CD),'') is null then 'N'
			when NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.STATUS_SOURCE),'') is null then 'N'
			when TMP_IFRS17_ICG_STORES_PRUEMAS.BEN_EXP_DATE is null then 'N'
			when (TMP_IFRS17_ICG_STORES_PRUEMAS.CB_START_DT > TMP_IFRS17_ICG_STORES_PRUEMAS.BEN_EXP_DATE AND TMP_IFRS17_ICG_STORES_PRUEMAS.STATUS_SOURCE = 'INFORCE')  then 'N'
			ELSE 'Y'
		end) = 'Y'
		;
		...
		update FOND_ID.FOND_IFRS17_ICG_STORES 
		set STATUS_SOURCE = TMP_IFRS17_ICG_STORES_PRUEMAS.STATUS_SOURCE, INFORCE_FLAG = TMP_IFRS17_ICG_STORES_PRUEMAS.INFORCE_FLAG, UPDATED_DATE=current_timestamp
		from STAG_ID.TMP_IFRS17_ICG_STORES_PRUEMAS 
		where UPPER(TRIM(FOND_IFRS17_ICG_STORES.POLICY_NO)) = UPPER(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.POLICY_NO))
		AND FOND_IFRS17_ICG_STORES.BENEFIT_CD = TMP_IFRS17_ICG_STORES_PRUEMAS.BENEFIT_CD
		AND lower(trim(FOND_IFRS17_ICG_STORES.SUBSYSTEM)) in (lower(trim('PruEmas')),lower(trim('PruEmasSya')))
		AND TMP_IFRS17_ICG_STORES_PRUEMAS.ICG_FLAG = 'UPDATE EXISTING BUSINESS' 
		AND NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.INFORCE_FLAG),'') is not null 
		AND NULLIF(TRIM(TMP_IFRS17_ICG_STORES_PRUEMAS.STATUS_SOURCE),'') is not null 
		;
		...
		--------------------------------------------------
		--- Insert Table for MP Files based on valid records in ICG Stores
		---------------------------------------------------
		-- insert valid records
		SET @V_SEQNO 	= @V_SEQNO + 1;
		SET @V_START 	= convert(datetime,getDATE());
		SET @V_DESCRIPTION	= 'create table FOND_ID.FOND_IFRS17_MPFILES_PRUEMAS: ' + convert(varchar,@V_START,121);
		PRINT @V_DESCRIPTION;
		...
		INSERT INTO FOND_ID.FOND_IFRS17_MPFILES_PRUEMAS
		SELECT
		...
		,@BATCHDATESTR AS BATCHDATE
		,@GMT_START_DTTM AS ETL_PROCESS_DATE_TIME
		from STAG_ID.STAG_PRUEMAS_STAG_ETL1_POLICY_RELATED PRUEMAS
		LEFT JOIN  (
			select x.*
			, row_number() over(partition by POLICY_NO 
			order by CB_START_DT desc, CB_END_DT desc) as idx 
			FROM FOND_ID.FOND_IFRS17_ICG_STORES x
			where lower(trim(x.SUBSYSTEM)) in (lower(trim('PruEmas')),lower(trim('PruEmasSya')))
			AND PRODUCT_CD = BENEFIT_CD
		) a
		ON (CASE WHEN NULLIF(TRIM(PRUEMAS.POLICY_ID),'') IS NOT NULL THEN UPPER(TRIM(PRUEMAS.POLICY_ID)) ELSE 'N/A' END) = UPPER(TRIM(a.POLICY_NO))
		AND a.idx = 1
		...
		AND UPPER(TRIM(c.SYSTEM)) in (UPPER(TRIM(CAST('PruEmas' AS VARCHAR(20)))),UPPER(TRIM(CAST('PruEmasSya' AS VARCHAR(20)))))
		;
