...
dict_list = []
#hardcoded_file_name = ["USP_LOAD_ETL4_LIFEASIA_PARALLEL_STAG.sql"] #get desired file name
...
df = df.map(lambda x: x.replace(")","").replace("_'","").replace("'',","").replace("'","").replace(":",""))
df = df.map(lambda x: x.replace("+@V_DRIVER_PERIOD","").replace("+_HIS","")) #ETL5
df = df.map(lambda x: x.replace("+@BATCH_NUMBER+","").replace(",U","").replace(",@BATCH_NUMBER","")) #ETL4
